...
const toggleMenu = (sessionId: string) => {
  ...
  fetch(`https://web-production-93bbb.up.railway.app/api/chat/sessions?email=${encodeURIComponent(userEmail)}`)
    .then(res => res.json())
    .then(data => {
      setSessions(data);
...
const handleRename = async (sessionId: string) => {
  ...
  if (!newTitle) return;
  await fetch(`https://web-production-93bbb.up.railway.app/api/chat/rename_session`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ session_id: sessionId, title: newTitle }),
  });
  const updated = await fetch(`https://web-production-93bbb.up.railway.app/api/chat/sessions?email=${userEmail}`).then(res => res.json());
  setSessions(updated);
};

const handleDelete = async (sessionId: string) => {
  if (!confirm("Are you sure you want to delete this chat?")) return;
  await fetch(`https://web-production-93bbb.up.railway.app/api/chat/delete_session`, {
    method: "DELETE",
    headers: { "Content-Type": "application/json" },
  ...
  useEffect(() => {
    ...
    fetch(`https://web-production-93bbb.up.railway.app/api/chat/messages?session_id=${selectedSession}`)
      .then(res => res.json())
  ...
  const startNewChat = async () => {
    const res = await fetch('https://web-production-93bbb.up.railway.app/api/chat/start_session', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await fetch('https://web-production-93bbb.up.railway.app/api/chat/ask', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          question: newMessage,
