...
export default function ComplaintsPage() {
  ...
  const statusMap: Record<string, { label: string; color: string }> = {
    under_checking: { label: 'Received', color: 'bg-blue-100 text-blue-800' },
    ...
    done: { label: 'Responded', color: 'bg-gray-100 text-gray-800' },
  };
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await fetch(`${api}/api/student/showcomplaints?student_email=${email}`);
  ...
  return (
    ...
    >
      ...
      {loading ? (
        ...
        <p className="text-white mt-20">You haven't submitted any complaints yet.</p>
      ) : (
            ...
            <tbody>
              {complaints.map((complaint, idx) => (
                <tr
                  ...
                  onClick={() => router.push(`/student_complaint/${complaint.complaint_id}`)}
                ...
                >
                  ...
                  <td className="px-6 py-3 capitalize">{complaint.complaint_type}</td>
                  <td className="px-6 py-3 capitalize">
                    {complaint.complaint_dep === "public" ? "Hidden" : "Shown"}
                  </td>
                  <td className="px-6 py-3">{formatDate(complaint.complaint_created_at)}</td>
                  <td className="px-6 py-3">
                    {statusMap[complaint.complaint_status] ? (
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${statusMap[complaint.complaint_status].color}`}>
                        {statusMap[complaint.complaint_status].label}
                      </span>
                    ) : (
